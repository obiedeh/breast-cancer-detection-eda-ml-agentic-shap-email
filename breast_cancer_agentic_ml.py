...
def train_logistic_regression():
    print("\n" + "="*80)
    print("💠 LOGISTIC REGRESSION — TRAINING & TUNING")
    print("="*80)
...
def train_random_forest():
    print("\n" + "="*80)
    print("🌲 RANDOM FOREST — TRAINING & TUNING")
    print("="*80)
...
def train_svm():
    print("\n" + "="*80)
    print("🧠 SVM (RBF Kernel) — TRAINING & TUNING")
    print("="*80)
...
def run_rfecv_and_xgb():
    ...
    """
    print("\n" + "="*80)
    print("🔍 RFECV FEATURE SELECTION + XGBOOST")
    print("="*80)
...
def summarize_and_choose_model(preferred: str = "XGBoost"):
    ...
    if preferred in trained_models:
        ...
        print(f"Preferred model not found — falling back to: {best_name}")
...
def send_email_to_doctor(
    ...
    smtp_user: str,
    smtp_password: str,
    subject: str = "Breast Imaging Case – Automated Triage Alert"
) -> None:
    """
...
def patient_triage_agent(
    ...
    if explainer is not None:
        try:
            ...
            for i in top_idx:
                direction = (
                    "↑ risk (toward malignant)" if shap_values_patient[i] > 0
                    else "↓ risk (toward benign)"
                )
    ...
    if email_attempted:
        try:
            send_email_to_doctor(
                ...
                subject=f"[ALERT] High-Risk Breast Imaging Case – Patient {patient_id}"
            )
            email_sent = True
        except Exception as e:
            email_error = str(e)
...
# ============================================================
# SECTION 8A: Simulated Agent Sensor & Actuator – Email Summary
# ============================================================
def sensor_actuator_email_alert(result, to_email="oncology_team@example.com"):
    ...
    subject = (
        f"[SIM-AGENT ALERT] Breast Imaging – {patient_id} MALIGNANT – Follow Up Needed."
    )
    ...
    body_lines.append("")
    body_lines.append(
        "(Simulation only – no real email was sent. This system is for research/education.)"
    )

    email_message = "\n".join(body_lines)

    print("\n" + "-" * 80)
    print(f"{COLOR_BOLD}{COLOR_MAGENTA}📧 SIMULATED AGENT EMAIL – SENSOR & ACTUATOR{COLOR_RESET}")
    print("-" * 80)
    print(email_message)
    print("-" * 80 + "\n")
...
def interactive_agent_session():
    ...
    if deployed_model is None or shap_explainer is None:
        print("\n" + "="*80)
        print("⚠️ Synthetic Test Lab requires a deployed model and SHAP explainer.")
        print("   Run:")
    ...
    header = f"{COLOR_CYAN}{COLOR_BOLD}*** Breast Imaging – Synthetic Patient Lab ***{COLOR_RESET}"
    print("\n" + header)
    ...
    # Feature Range Guide
    # ------------------------------------------------------------
    print(f"{COLOR_BOLD}{COLOR_BLUE}🧭 Feature Range Guide{COLOR_RESET}")
    ...
    # ------------------------------------------------------------
    print("\n" + "-"*80)
    print("🧪 Generating 10 synthetic patients...")
    print("-"*80)
    ...
    print("\n" + "="*80)
    print("📊 SYNTHETIC PATIENT FEATURE TABLE (TRANSPOSED — Top & Bottom 5 Features)")
    print("="*80)
    print(df_display_top.to_string())
    print("...\n")
    print(df_display_bottom.to_string())
    print("="*80)
    print(f"{COLOR_DIM}Rows = Features, Columns = Patients (P001…P00N){COLOR_RESET}")
    print(f"{COLOR_DIM}Enter a patient_id to run triage. Use 'table' to reprint.{COLOR_RESET}")

    # ------------------------------------------------------------
    # LOOP — ALLOW TESTING EACH PATIENT
    # ------------------------------------------------------------
    while True:
        ...
        if match.empty:
            print(f"{COLOR_RED}❗ patient_id '{pid}' not found. Try again.{COLOR_RESET}")
            continue
